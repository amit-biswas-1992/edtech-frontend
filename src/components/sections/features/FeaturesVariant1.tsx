...
const defaults = {
  ...
  items: [
    ...
    { title: "Affordable Fees", description: "Quality education accessible to all with flexible payment plans.", icon: "currency" },
    { title: "Global Recognition", description: "Earn certificates recognized by employers worldwide.", icon: "globe" },
  ],
};

function getIcon(icon: string, color: string) {
  const props = { className: "w-7 h-7", fill: "none", stroke: color, viewBox: "0 0 24 24", strokeWidth: 1.5 };
  switch (icon) {
    case "academic":
...
export default function FeaturesVariant1({ content, colorTheme }: SectionProps) {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {items.map((item: { title: string; description: string; icon: string }, i: number) => (
            <div
              key={i}
              className="p-7 rounded-xl text-center transition-all duration-300 hover:shadow-lg hover:-translate-y-1"
              style={{
            ...
            >
              ...
              <h3 className="text-lg font-semibold mb-3" style={{ color: colorTheme.primary }}>
                {item.title}
              </h3>
