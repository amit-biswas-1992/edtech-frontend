...
const defaults = {
  ...
  items: [
    { title: "Expert Faculty", description: "Learn from industry-leading educators with years of experience in their respective fields. Our faculty members are dedicated to nurturing the next generation of leaders.", icon: "academic" },
    { title: "Modern Curriculum", description: "Stay ahead with our constantly updated, industry-relevant courses designed in collaboration with leading companies and organizations.", icon: "book" },
    { title: "Flexible Learning", description: "Study at your own pace with online and offline options. Our hybrid model ensures you never miss a class regardless of your schedule.", icon: "clock" },
    { title: "Career Support", description: "Get placement assistance and career guidance from day one. Our dedicated career services team works tirelessly to connect you with top employers.", icon: "briefcase" },
  ],
};

function getIcon(icon: string, color: string) {
  const props = { className: "w-10 h-10", fill: "none", stroke: color, viewBox: "0 0 24 24", strokeWidth: 1.5 };
  switch (icon) {
    case "academic":
...
export default function FeaturesVariant2({ content, colorTheme }: SectionProps) {
              ...
              >
                ...
                <div className="flex-shrink-0">
                  <div
                    ...
                    style={{
                      background: `linear-gradient(135deg, ${colorTheme.primary}20, ${colorTheme.accent}20)`,
                      border: `2px solid ${colorTheme.primary}15`,
                    }}
                  >
                ...
                <div className={`flex-1 ${isEven ? "md:text-left" : "md:text-right"} text-center`}>
                  <h3 className="text-xl font-bold mb-3" style={{ color: colorTheme.primary }}>
                    {item.title}
                  </h3>
